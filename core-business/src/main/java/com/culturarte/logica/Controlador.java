...
public class Controlador implements IControlador {
    ...
    public void altaProponente(String nickname, String password, String nombre, String apellido, String email,
        ...
        mu.agregarUsuario(new Proponente(nickname, password, nombre, apellido, email, fechaNacimiento, imagen,
                direccion, linkWeb, bibliografia));
    }
    ...
    public DTProponente getDTProponente(String nickname) {
        // Datos usuario, datos proponente, Propuestas (nombre, estado, lista
        // colaboradores, monto recaudado, monto necesario)

        Proponente p = mu.getProponenteConPropuestas(nickname);

        DTProponente dtp = new DTProponente(p.getNickname(), p.getPassword(), p.getNombre(), p.getApellido(),
                p.getEmail(), p.getFechaNacimiento(), p.getImagen(), p.getDireccion(), p.getLinkWeb(),
                p.getBiografia());
    ...
    public void altaPropuesta(String titulo, String descripcion, String lugar, LocalDate fechaPrevista,
        ...
        Propuesta p = new Propuesta(titulo, descripcion, lugar, fechaPrevista, precioEntrada, montoNecesario,
                tipoRetornos, imagen, u, c);
    ...
    public DTPropuesta getDTPropuesta(String titulo) {
        ...
        if (p != null) {
            ...
            for (Estado est : p.getHistorialEstados()) {
                histEstado.add(new DTEstado(est.getEstado().toString(), est.getFecha().toString(),
                        est.getHora().format(formatter)));
            }
            ...
            dtp = new DTPropuesta(p.getTitulo(), p.getDescripcion(), p.getLugar(), p.getFechaPrevista(),
                    p.getPrecioEntrada(), p.getMontoNecesario(), p.getImagen(), p.getNicknameColaboradores(),
                    p.getProponenteNick(), p.getEstadoActual().getEstado(), nombreCategoria, histEstado,
                    p.getMontoRecaudado(), comentariosDT);
        }
        return dtp;
    }
    ...
    public boolean colaboradorPuedeComentar(String colaborador, String tituloPropuesta) {
        ...
        if (p.getEstadoActual().getEstado() != TipoEstado.FINANCIADA)
            return false;
    ...
    public void dejarDeSeguirUsuario(String nickSeguidor, String nickSeguido) throws UsuarioNoSeguido {
        ...
        if (usuarioAEliminarDeSeguidos != null && usuarioAEliminarDeSeguidores != null) {
            ...
            throw new UsuarioNoSeguido(
                    "El usuario con nickname: " + nickSeguidor + ", no sigue al usuario con nickname: " + nickSeguido);
        }
    }
    ...
    public DTUsuario getDTUsuario(String nickname) {
        ...
        if (usu == null)
            usu = mu.buscarUsuarioPorEmail(nickname);
        if (usu == null)
            return null;
        ...
        DTUsuario dtu = new DTUsuario(usu.getNickname(), usu.getNombre(), usu.getApellido(), usu.getEmail(),
                usu.getFechaNacimiento(), usuariosSeguidos, usuariosSeguidores, tipo, usu.getImagen(),
                propuestasSeguidas);
    ...
    public boolean verificarPassword(String password, String nick) {
        ...
        if (usu == null) {
            usu = mu.buscarUsuarioPorEmail(nick);
        }
        if (usu == null) {
            return false;
        }
    ...
    public void modificarPropuesta(String titulo, String descripcion, String lugar, LocalDate fechaPrevista,
            Float precioEntrada,
            Float montoNecesario, String imagen, String proponente, String categoria, String nuevoEstado)
            throws DatosIncorrectos {

        if (titulo.isEmpty())
            throw new DatosIncorrectos("El titulo no puede ser vacío");
        if (lugar.isEmpty())
            throw new DatosIncorrectos("El lugar no puede ser vacío");
        if (fechaPrevista == null)
            throw new DatosIncorrectos("La fecha no puede ser vacía");
        if (proponente.isEmpty())
            throw new DatosIncorrectos("El proponente no puede ser vacío");
        if (categoria.isEmpty())
            throw new DatosIncorrectos("La categoría no puede ser vacía");
        if (nuevoEstado.isEmpty())
            throw new DatosIncorrectos("El estado no puede ser vacío");
    ...
    public ArrayList<DTColaboracion> getColaboracionesSinPago(String nickColaborador) {
        java.util.List<Colaboracion> colaboraciones = mcol.getColaboracionesSinPago(nickColaborador);
        ArrayList<DTColaboracion> resultado = new ArrayList<>();

        for (Colaboracion colab : colaboraciones) {
            DTColaboracion dtColab = new DTColaboracion(
                    colab.getColaborador().getNickname(),
                    colab.getPropuesta().getTitulo(),
                    colab.getFechaAporte(),
                    colab.getHoraAporte(),
                    colab.getMonto(),
    ...
    public void registrarPago(DTPago dtPago, String nickColaborador, String tituloPropuesta) throws Exception {
        ...
        Propuesta propuesta = mp.getPropuesta(tituloPropuesta);
        if (propuesta == null) {
        ...
        } else if (dtPago.getTipoPago() == com.culturarte.logica.enums.TipoPago.TRANSFERENCIA) {
            pago.setNombreBanco(dtPago.getNombreBanco());
            pago.setNumeroCuenta(dtPago.getNumeroCuenta());
        } else if (dtPago.getTipoPago() == com.culturarte.logica.enums.TipoPago.PAYPAL) {
            pago.setNumeroCuenta(dtPago.getNumeroCuenta());
        }
